...
export default function AgentApplyipo2(props) {
  // declaration
    ...
    let [respvalue,setrespvalue]=useState("")
...
// end of of mine
        ...
        httpClient.UPLOAD("POST","api/applicant/create/agent",finalValue,finalValue.citizenshipFrontImage,finalValue.citizenshipBackImage,null,{ headers: {"Authorization" : `Bearer ${token}`} })
        .then(response=>{

          notify.success(finalValue.kitta+" no. of shares successfully applied")
          let value=JSON.parse(response);
          
          let finaldata=value.data
        setTimeout(() => {
          props.history.push("/voucherupload",finaldata)
        }, 5000);
          
          // setrespvalue(resp.data.data)
          // history.push("/agent")
